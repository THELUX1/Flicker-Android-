...
async function getSeriesTrailer(seriesId, seriesTitle) {
  try {
    ...
    const [tmdbVideos, tmdbAlternative, youtubeSearch] = await Promise.allSettled([
      ...
      fetch(`https://api.themoviedb.org/3/tv/${seriesId}/videos?api_key=${TMDB_API_KEY}&language=es-ES`).then(res => res.json()),
      
      // 2. Búsqueda alternativa en TMDB (inglés)
      fetch(`https://api.themoviedb.org/3/tv/${seriesId}/videos?api_key=${TMDB_API_KEY}&language=en-US`).then(res => res.json()),
      
      // 3. Búsqueda en YouTube por título (fallback)
      searchYouTubeTrailer(seriesTitle + " serie trailer oficial")
    ]);
    ...
    if (tmdbVideos.status === 'fulfilled' && tmdbVideos.value.results) {
      ...
      if (trailer) {
        console.log(`✅ Trailer encontrado en TMDB (ES) para: ${seriesTitle}`);
        return trailer;
      }
    }
    ...
    if (tmdbAlternative.status === 'fulfilled' && tmdbAlternative.value.results) {
      ...
      if (trailer) {
        console.log(`✅ Trailer encontrado en TMDB (EN) para: ${seriesTitle}`);
        return trailer;
      }
    }
    ...
    if (youtubeSearch.status === 'fulfilled' && youtubeSearch.value) {
      console.log(`✅ Trailer encontrado en YouTube para: ${seriesTitle}`);
      return youtubeSearch.value;
    }

    console.log(`❌ No se encontró trailer para: ${seriesTitle}`);
    return null;
  } catch (err) {
    console.error('Error en búsqueda de trailer de serie:', err);
    return null;
  }
}
...
async function loadSeriesDetails(series) {
  try {
    ...
    const trailerHTML = generateTrailerHTML(trailer, series.title);
      ...
      <div class="trailer-placeholder">
        <div style="text-align: center; padding: 60px 20px;">
          ...
          <div style="font-size: 1rem; color: var(--texto-terciario); max-width: 300px; margin: 0 auto; line-height: 1.5;">
            No hemos encontrado un tráiler para esta serie
          </div>
        </div>
      </div>
    `;
